import os
from utils.place_info_search import GooglePlaceSearchTool, TavilPlaceSearchTool
...
class PlaceSearchTool :
    ...
    def __init__(self) :
        ...
        self.tavily_search  = TavilPlaceSearchTool()
        self.place_search_tool_list = self._setup_tools()

    def _setup_tools (self) -> List :
        """
        Setup tool for the place search tool implementations
        """
        @tool
        ...
        def search_activities(place : str) -> str :
            try : 
                activity_results = self.google_places_search.google_search_activity(place)

                if activity_results :
                    return f"Following are the activities in and around {place} as suggested by google : {activity_results}."
                
            except Exception as e :
                tavily_result = self.tavily_search.tavily_search_activity(place)

                return f"Google cannot find the details due to {e}. \nFollowing are the activities related to {place} : {tavily_result}."
